...
const buffingStageServiceApi = {
    ...
    getSuggestedPatterns: (orderCasingId: number) =>
        api.get(
            `/tread-patterns/suggestions?orderCasingId=${orderCasingId}`
        ),

    approveRejectPreBuffing: (data: any) =>
        api.post(
            "/pre-buffing/approve-reject",
            data
        ),
    ...
    getPostBuffingRejectionReason: () =>
        api.get(
            "/rejection-reasons?currentStage=7&currentSubstage=72"
        ),

    getMachines: () =>
        api.get("/machines?stage=7"),
    ...
    approveRejectPostBuffing: (
        data: any
    ) =>
        api.post(
            "/post-buffing/approve-reject",
            data
        ),
};
